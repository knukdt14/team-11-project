...
1. [이 문서의 위치](#1-이-문서의-위치)
2. [왜 이 주제인가](#2-왜-이-주제인가)
3. [이번 프로젝트가 하는 것 / 하지 않는 것](#3-이번-프로젝트가-하는-것--하지-않는-것-)
4. [핵심 원칙 — CV는 판정기가 아니라 입력 어댑터](#4-핵심-원칙--cv는-판정기가-아니라-입력-어댑터-)
5. [화면에 반드시 보여야 하는 것](#5-화면에-반드시-보여야-하는-것-)
6. [시연 시나리오 (5분)](#6-시연-시나리오-5분)
7. [아키텍처](#7-아키텍처)
8. [기술별 역할 분담](#8-기술별-역할-분담)
9. [웹서비스 심화 포인트](#9-웹서비스-심화-포인트--이번-모듈의-승부처)
10. [RAG / 지식베이스 설계](#10-rag--지식베이스-설계)
11. [API 계약](#11-api-계약)
12. [폴더 구조](#12-폴더-구조)
13. [평가표 매핑](#13-평가표-매핑)
14. [역할 분담](#14-역할-분담-4인-1팀)
15. [개발 순서와 게이트](#15-개발-순서와-게이트)
16. [개발 규칙](#16-개발-규칙)
17. [리스크와 폴백](#17-리스크와-폴백)
18. [실행 방법](#18-실행-방법)
19. [저작권·면책](#19-저작권면책)
...
| 문서 | 내용 | 상태 |
|---|---|---|
| **`README.md`** (이 문서) | **확정된 프로젝트 방향 · 범위 · 계약 · 일정** | ★ 단일 진실 |
| `PIPELINE.md` | CV 알고리즘·수식·의사코드 상세 | 참고용 (기술 레퍼런스, **범위는 이 문서가 우선**) |
| `README (1).md` | 초안. 범위가 넓고 "구현 완료" 표시가 실제와 다름 | ⚠️ **폐기 예정 — 삭제하세요** |

> ⚠️ **현재 저장소에는 코드가 없습니다.** 아래 모든 항목은 `⬜ 미구현`에서 시작합니다.
> `README (1).md`의 ✅ 표시는 사실이 아니므로 신뢰하지 마세요.
...
> 💬 **발표에서 이렇게 말합니다:**
> *"임의의 사고영상을 넣으면 정확한 과실비율이 나오는 시스템은 석사 논문 스코프입니다.
> 저희는 CV를 **판정기가 아니라 입력 어댑터**로 정의해서, 이번 모듈 기간에 **완결되는 웹서비스**를 만들었습니다."*
>
> 범위를 좁힌 것은 실패가 아니라 **설계 결정**입니다. 그렇게 설명하면 감점이 아니라 가점입니다.

---

## 4. 핵심 원칙 — CV는 판정기가 아니라 입력 어댑터 ★
...
```
① [홈]  서비스 소개 · 백엔드 연결 상태 배지 · 면책 문구
        └ 예시 영상 3개가 카드로 깔려 있음 (사전 처리 완료본)

② [영상 분석]  카드 클릭 또는 직접 업로드
        └ st.status 로 단계가 실시간으로 채워짐
           ✓ 영상 정규화 (10fps, 720p)
           ✓ 차량 검출·추적  → 4대 감지 (track 1,2,3,7)
           ✓ 궤적 생성
           ✓ 충돌 감지      → t_c = 4.2s (score 0.71)
           ✓ 특징 추출      → 교차각 87°, 상대 좌회전, 야간

③ 주석 영상 재생 + 충돌 순간 3컷 + 궤적 요약 이미지

④ 자동 채워진 체크박스 + 신뢰도 배지
        ☑ 야간            (0.95) 밝기 평균 52, 헤드라이트 3개
        ☑ 상대 선진입      (0.62) 교차영역 진입 0.8초 선행
        ☐ 과속            (0.20) 속도 추정 신뢰도 낮음
   되묻기 칩  [신호등이 있는 교차로였나요?] [상대가 과속했나요?]

⑤ [이 내용으로 상담하기] → 상담 대시보드로 합류
        ◉ 게이지        나 30 : 상대 70
        ◉ 워터폴        30 → +10(야간) → −10(선진입) → 30
        ◉ 근거 도표      차25.png + 인정기준 p.148 지면 렌더
        ◉ 유사 사례      4개 PDF에서 검색된 유사 기준 3건
        ◉ 판례 · 법조항   도로교통법 제26조
        ◉ Agent trace   4단계 실행 로그 (펼치기)

⑥ 수정요소 토글 ☐ 과속 → ☑  → 즉시 재계산 (LLM 미호출, 체감 0초)

⑦ [결과 리포트 저장] → Markdown/PDF 다운로드

⑧ [지식베이스] 탭에서 PDF 1개 추가 업로드 → 인덱싱 → 검색되는 것 확인
```

> ⏱️ **시연 안정성**: 예시 영상 3개는 **사전 처리해 결과를 캐시**합니다.
> 라이브 처리는 **짧은 영상 1개만** 보여주고, 나머지는 캐시본으로 즉시 재생합니다.
...
## 9. 웹서비스 심화 포인트 ★ 이번 모듈의 승부처

> 평가표에 **"기존 수업에서 다루지 않은 UI 컴포넌트 / API를 추가했는가"**가 명시돼 있고,
> Module 12에서 LLM/RAG는 이미 다뤘습니다. **무게중심은 사용성과 웹서비스 완성도입니다.**
...
| 평가 항목 | 배점 | 이 프로젝트에서 |
|---|:---:|---|
| **Frontend / Streamlit** | 20 | 5페이지 구조 · `st.video` · `st.status` 실시간 진행률 · plotly 게이지/워터폴 · 프레임 3컷 · 신뢰도 배지 · 되묻기 칩 · 즉시 재계산 토글 · 리포트 다운로드 |
| **Backend / FastAPI** | 20 | 라우터/서비스 분리 · `BackgroundTasks` 비동기 잡 + 폴링 · `StaticFiles` 2개 · `lifespan` 모델 1회 로드 · Pydantic 계약 · `/health` · `/recalculate` · Tool Calling |
| **LLM / RAG** | 20 | Chroma 벡터스토어 · 4개 PDF 다중 업로드 · **도표 단위 청킹** · 메타필터 하이브리드 검색 · 환각 차단 프롬프트 · HF 로컬 ⇄ API 스위칭 비교 |
| **발표 및 시연** | 20 | 4인 역할 분담 명확 · **"CV를 판정기가 아닌 입력 어댑터로 정의"라는 설계 결정 스토리** · 어려웠던 점(라벨 없는 데이터, 각도 부호 버그) |
| **프로젝트 완성도** | 20 | 영상 → 과실비율 **한 화면 완주** · 프론트/백/AI 모듈 분리 · 텍스트 경로 폴백 |

> 📌 **"프론트엔드/백엔드 구조의 분리에 대한 설명 필수"**(강의자료 p.168)
> → 발표에서 *"Streamlit에는 연산이 한 줄도 없습니다. YOLO·임베딩·검색·계산 전부 FastAPI에 있고,
> 프론트는 HTTP/JSON으로만 대화합니다"* 를 아키텍처 그림과 함께 명시하세요.
...
| 담당 | 역할 | 주요 산출물 |
|:---:|---|---|
| **팀장** | 프로젝트 총괄 · KB 구축(4개 PDF → 구조화) · 발표자료 | `kb_builder/*`, `kb.sqlite`, 발표 PPT |
| **Frontend** *(일감 최대)* | Streamlit 5페이지 · 시각화 · 영상 분석 화면 · 사용성 | `frontend/*` |
| **Backend** — 오현택 | FastAPI 설계 · 비동기 잡 · 도구 6개 · 에이전트 | `backend/routers/*`, `backend/services/*` |
| **CV / AI** | YOLO·ByteTrack·궤적·충돌·특징·**주석 렌더** + LLM 공급자 | `backend/services/cv/*`, `llm.py`, `tests/test_cv_features.py` |

> 발표는 4명이 각자 파트를 **5분씩(총 20분)** 담당합니다.
> ※ 역할은 자유롭게 변경 가능하지만, **API 계약(§11)만은 첫날 함께 확정**해야 병행 개발이 됩니다.

### 담당별 ⭐ 승부처

- **Frontend** — CV 잡 진행률을 `st.status`로 실시간 스트리밍. "정규화 → 검출 → 궤적 → 충돌 → 특징"이 눈앞에서 차례로 채워지는 연출. **시연 임팩트가 여기서 나옵니다.**
- **Backend** — 숫자를 LLM에서 분리한 설계. `apply_modifiers()`를 순수 함수로 두어 환각으로 비율이 틀릴 가능성을 **구조적으로 제거**. 같은 도구 위에 결정론적 경로와 에이전트 경로를 **동시에** 구현.
- **CV / AI** — YOLO 결과를 **화면에 그려서 보여주는 것**까지가 담당 범위. 내부 데이터로만 쓰면 이 프로젝트는 성립하지 않습니다. + 로컬 sLLM vs API tool calling 성공률 비교.
- **팀장** — 다중 문서 통합. 인정기준 + 비정형기준 3종 + 도로교통법을 **하나의 KB 스키마로 정규화**. 도표 단위 청킹의 검색 정확도 비교 실험.
...
cp .env.example .env                            # LLM_PROVIDER=rule 이면 수정 없이 동작

python -m kb_builder.build_sqlite               # data/kb.sqlite 생성
python -m kb_builder.extract_images             # 도표 이미지 · PDF 지면 렌더
python -m kb_builder.build_vector               # 벡터 인덱스 (없으면 키워드 검색으로 degrade)
```
...
| 주소 | 내용 |
|---|---|
| http://localhost:8501 | 대시보드 |
| http://localhost:8000/docs | API 문서 (발표 산출물) |
...
`.env`의 `LLM_PROVIDER` 한 줄만 바꾸면 됩니다. 코드 수정 없습니다.

| 값 | 동작 | 필요 조건 |
|---|---|---|
| `rule` | 규칙 기반 (개발·시연 안전판) | 없음 — GPU·키 불필요 |
| `local` | **HF Transformers 로컬 모델** (Qwen2.5-7B-Instruct 등) | GPU, `transformers`·`torch` |
| `api` | 오픈 API | `OPENAI_API_KEY` |

에이전트 모드(tool calling)는 `api`에서 가장 안정적이며, 나머지에서는 **고정 파이프라인으로 자동 폴백**합니다.

---

## 19. 저작권·면책

- **자동차사고 과실비율 인정기준** (제10차 개정, 2023.6) — 손해보험협회.
  PDF에 *"개인 참고용 활용, 책자 발간 및 배포용으로 이용 불가"*가 명시돼 있습니다.
  → 학습·포트폴리오 목적 사용은 OK. **원본 PDF를 GitHub에 커밋하지 마세요.**
  → README·발표자료·앱 화면 하단에 **"출처: 손해보험협회 / 개인 학습용"** 명기.
- **도로교통법** — 국가법령정보센터 (law.go.kr)
- **과실비율 분쟁 통계** — 보험연구원 보고서 / 제도 변경 보도 — 뉴시스
- **사고 영상** — 저작권·초상권을 확인한 것만 사용. 공개 데이터셋(CCD·DoTA·A3D)은 각 라이선스를 따릅니다.
  **영상을 저장소에 커밋하지 마세요.**
...
- [ ] **★ 고정 CCTV 사고영상 2~3개 확보** ← 여기서 프로젝트 성패가 갈립니다. 가장 먼저
- [ ] 메인 PDF 4종 다운로드 → `data/raw_pdf/` (**커밋 금지**)
- [ ] `parse_pdf --diagnose`로 수정요소 표가 텍스트인지 이미지인지 확인
- [ ] MVP 대상 사고유형 2~3개 합의 (권장: 무신호교차로 직진 vs 좌회전)
- [ ] **`schemas.py` API 계약 확정 후 커밋** ← 이게 되어야 4명이 병렬로 일합니다
- [ ] GPU 환경 확인 (로컬 / Colab)
- [ ] D-day 기준으로 §15 개발 순서에 날짜 채우기
