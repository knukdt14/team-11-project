...
> 모델 비교와 LLM 대화까지 맡기지 않습니다. 데이터에서 검색 결과가 나오는 기본 경로만 책임집니다.
...
> 1번은 **데이터를 검색 가능하게 만드는 사람**, 2번은 **검색 성능을 높이고 검증하는 사람**으로 구분합니다.
...
> LLM 에이전트 전체를 혼자 만드는 것이 아니라, 정확한 계산과 상담 상태 관리에 집중합니다.
...
> Docker는 초기에 기본 파일만 만들고, 프론트가 완성된 후 최종 이미지를 제작합니다.
