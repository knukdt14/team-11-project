...
def main() -> None:
    ...
    for r in 답있음:
        ...
        # 정답이 여럿일 수 있습니다(제목이 같은 도표 등). 세미콜론으로 구분하고,
        # 그중 **가장 높은 순위**를 그 문항의 순위로 봅니다.
        golds = [g.strip() for g in r["gold_id"].split(";") if g.strip()]
        ranks_ = [ids.index(g) + 1 for g in golds if g in ids]
        rank = min(ranks_) if ranks_ else None
        ranks.append(rank)
        ...
        표시 = golds[0] + (f" 외{len(golds)-1}" if len(golds) > 1 else "")
        print(f"{r['query'][:42]:<44} {표시:<22} {mark}")
